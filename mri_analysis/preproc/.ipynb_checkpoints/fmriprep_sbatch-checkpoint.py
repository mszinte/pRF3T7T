...
singularity_cmd = "singularity run --cleanenv{tf_bind} -B {main_dir}:/work_dir {simg} --fs-license-file /work_dir/freesurfer/license.txt /work_dir/{project_dir}/ /work_dir/{project_dir}/derivatives/fmriprep/ participant --participant-label {sub_num} -w /work_dir/temp_data/ --bold2t1w-dof {dof} --ignore sbref --output-spaces T1w fsnative fsaverage MNI152NLin2009cAsym:res-1{hcp_cifti} --low-mem --mem-mb 64000 --nthreads {nb_procs:.0f}{anat_only}{use_aroma}{use_fmapfree}{use_skip_bids_val}".format(tf_bind=tf_bind, main_dir=main_dir, project_dir=project_dir,
